...
Valid Login Test
    ...
    Click Element    xpath=//a[.//p[text()='Properti']]
    Sleep    3s
    Click Element    xpath=//button[@data-target='modal1']
    Sleep    3s
    Click Element    xpath=//button[.//p[text()='JAWA BARAT']]
    Sleep     3s
    Click Element    xpath=//button[.//p[text()='Kab. Bekasi']]
    Sleep    3s
    Click Element    xpath=//button[.//p[text()='Bantarjaya']]
    Sleep    3s
    Click Element    xpath=//button[.//p[text()='2976-Dwelling House']]
    Sleep    3s
    Click Element    xpath=//button[.//p[text()='100% Baja']]
    Sleep    3s
    Input Text    id=nilaiBangunan     100000000
    Sleep    3s
    Input Text    id=isiBangunan     100000000
    Sleep    3s
    Input Text    id=stockBangunan     100000000
    Sleep    3s
    Input Text    id=mesinBangunan     100000000
    Sleep    3s
    Click Element    xpath=//button[contains(text(), 'Simpan')]
    Sleep    3s
    Click Element    xpath=//button[.//p[text()='Daerah yang tidak pernah mengalami banjir atau pernah mengalami banjir(ketinggian kurang lebih 30 cm)']]
    Sleep    3s
    # Input Text    id=hargaProperti     400000000
    Click Button    id=submit
    Sleep    15s
    Scroll Element Into View    xpath=//button[@data-index="0"]
    Click Button    xpath=//button[@data-index="0"]
    Sleep    3s
    Click Button    xpath=//button[@data-asuransi="55"]
    Sleep    3s
    Click Element    css=.rounded-lg.py-3.font-semibold.text-base.indotekno-bg.text-white.w-full
    Sleep    3s
    # ==============
    ...
    Press Keys    NONE    ESC
    Sleep    3s
    Input Text    id=nomer-ktp    124567898765
    Sleep    3s
    Input Text    id=nama   TEST IT ARI
    Sleep    3s
    Click Button    id=btn-data2
    Sleep    3s
    Click Element    xpath=//span[@aria-label='March 22, 2025']
    Sleep    3s
    Click Button    id=saveTgl
    Sleep    3s
    Select Radio Button    jk    Laki-laki
    Sleep    3s
    Input Text    id=nomer-hp    082111371287
    Sleep    3s
    ...
    Sleep    3s
    Select Radio Button    hard    Yes
    Sleep    3
    Click Button    id=getDataButton
    Sleep    3s
    # ==============
    Click Element    xpath=//button[@data-target='modal2']
    Sleep    3s
    Click Element    xpath=//span[@aria-label='March 22, 2025']
    Sleep    3s
    Click Button    id=saveTgl
    ...
    Press Keys    NONE    ESC
    Sleep    3s
    Click Button    id=btn-upload-car5
    Sleep    3s
    Choose File    id=uploadInput5     ${FILE}
    Press Keys    NONE    ESC
    Sleep    3s
    Click Button    id=getDataButton
    Sleep    3s
